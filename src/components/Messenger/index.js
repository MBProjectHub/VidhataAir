...
export default class Messenger extends React.Component {
                  ...
                  this.setState(
                    {
                      approvals: a.val(),
                      bookings: b.val(),
                      myApprovals: u.val()[fire.auth().currentUser.uid]
                        .approvals
                        ? u.val()[fire.auth().currentUser.uid].approvals
                        : {},
                      myBookings: u.val()[fire.auth().currentUser.uid].bookings
                        ? u.val()[fire.auth().currentUser.uid].bookings
                        : {},
                      users: u.val()
                    },
                    () => this.loadConvos()
  ...
  newBooking() {
    ...
    fire
      .database()
      .ref("/users/" + fire.auth().currentUser.uid + "/bookings")
      .update(temp);
  ...
  stageClick(label) {
    ...
    if (label == steps[0]) {
      ...
      label == steps[1] &&
      this.state.bookings.active[this.state.currentSelected].options.opts
    ) {
      fire
        .database()
        .ref("/bookings/active/" + this.state.currentSelected)
      ...
      label == steps[2] &&
      cur &&
      cur.confirmation.details != "-" &&
      cur.options.status != 3 &&
      cur.options.status != -1 &&
      cur.options.status != 0
    ) {
      fire
        .database()
        .ref("/bookings/active/" + this.state.currentSelected)
        ...
        >
          <Container style={{ padding: 0 }}>
            ...
            >
              ...
              <Col>
                <div>
                  {this.state.currentProgressStage == 1.5
                    ? ""
