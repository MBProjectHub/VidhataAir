...
export default class ConversationSearch extends React.Component {
  ...
  static getDerivedStateFromProps(props, state) {
    if (
      ...
      return {
        ...
        threadIds: Object.keys(props.myBookings),
        users: props.users
      };
    return null;
  }
  ...
  search(searchText) {
    ...
    for (let i = 0; i < this.state.threadIds.length; i++) {
      ...
      if (
        (
          item.initId.split("_")[3] +
          "-" +
          item.initId.split("_")[2] +
          "-" +
          item.initId.split("_")[1]
        ).includes(searchText) ||
        (item.request.details != "-" &&
          item.request.details.arr.toLowerCase().includes(searchText)) ||
        (item.request.details != "-" &&
          item.request.details.dept.toLowerCase().includes(searchText)) ||
        (item.confirmation.details != "-" &&
          item.confirmation.details.airline.toLowerCase().includes(searchText))
      )
